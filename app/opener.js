(function(global){
	 ...
	 */
	var open = function(rootUrl, config){
		//通过 window.name 属性传递的数据
		var nameData = {
			fileUrl: config.fileUrl,
			tableKey: config.tableKey,
			tableCaption: config.tableCaption||config.tableKey,
			storageIdentify: config.storageIdentify || 0,	//用于区分存储
			attributes: config.attributes || {}
		};

		//打开新窗口, 使用 window.name 传递数据
		var popup = window.open(rootUrl+"/app.html#" + (new Date()).getTime(), JSON.stringify(nameData));
		if (popup) {
			//正常弹出窗口
		} else {
			$.error("无法打开新的浏览器窗口显示透视表，请检查浏览器设置，允许浏览器弹出窗口");
		}
	};
 ...
 * Latest commit 2a76286 on 22 Apr 2018: https://github.com/douglascrockford/JSON-js/commit/2a76286e00cdc1e98fbc9e9ec6589563a3a4c3bb
 */
//  json2.js
//  2017-06-12
//  Public Domain.
